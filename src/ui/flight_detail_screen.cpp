...
constexpr size_t kRouteLabelLen = 54;  // "IATA, " + airport name (max 47 chars + null)
...
void resolveRouteLabels(const services::adsb::Aircraft& ac, char* origin, size_t origin_len,
  ...
  if (ac.route_origin[0] != '\0') {
    const char* code = (ac.origin_iata[0] != '\0') ? ac.origin_iata : ac.route_origin;
    if (ac.origin_name[0] != '\0') {
      snprintf(origin, origin_len, "%s, %s", code, ac.origin_name);
    } else {
      strncpy(origin, code, origin_len - 1);
      origin[origin_len - 1] = '\0';
    }
  }
  if (ac.route_dest[0] != '\0') {
    const char* code = (ac.dest_iata[0] != '\0') ? ac.dest_iata : ac.route_dest;
    if (ac.dest_name[0] != '\0') {
      snprintf(dest, dest_len, "%s, %s", code, ac.dest_name);
    } else {
      strncpy(dest, code, dest_len - 1);
      dest[dest_len - 1] = '\0';
    }
  }
}
