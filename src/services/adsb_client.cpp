...
void applyRouteFieldsByCallsignImpl(const char* callsign, const char* airline,
  ...
  for (size_t i = 0; i < s_aircraft_count; ++i) {
    ...
    if (origin_iata != nullptr && origin_iata[0] != '\0') {
      strncpy(ac.origin_iata, origin_iata, sizeof(ac.origin_iata) - 1);
      ac.origin_iata[sizeof(ac.origin_iata) - 1] = '\0';
    }
    if (dest_iata != nullptr && dest_iata[0] != '\0') {
      strncpy(ac.dest_iata, dest_iata, sizeof(ac.dest_iata) - 1);
      ac.dest_iata[sizeof(ac.dest_iata) - 1] = '\0';
    }
    if (origin_name != nullptr && origin_name[0] != '\0') {
      strncpy(ac.origin_name, origin_name, sizeof(ac.origin_name) - 1);
      ac.origin_name[sizeof(ac.origin_name) - 1] = '\0';
    }
    if (dest_name != nullptr && dest_name[0] != '\0') {
      strncpy(ac.dest_name, dest_name, sizeof(ac.dest_name) - 1);
...
void applyRouteFieldsByCallsign(const char* callsign, const char* airline,
  ...
  applyRouteFieldsByCallsignImpl(callsign, airline, origin, dest,
                                  origin_iata, dest_iata, origin_name, dest_name);
}
